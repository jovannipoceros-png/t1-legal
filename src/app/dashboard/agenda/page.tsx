  ...
  const generarReporte = async () => {
    ...
    try {
      ...
      const lineas = [
        ...
        obligaciones.filter((o:any) => getDiasRestantes(o.fecha_limite) < 0).length > 0 ? `⚠️  OBLIGACIONES VENCIDAS: ${obligaciones.filter((o:any) => getDiasRestantes(o.fecha_limite) < 0).length}` : null,
        obligaciones.filter((o:any) => { const d = getDiasRestantes(o.fecha_limite); return d >= 0 && d <= 7 }).length > 0 ? `⏰ VENCIMIENTOS ESTA SEMANA: ${obligaciones.filter((o:any) => { const d = getDiasRestantes(o.fecha_limite); return d >= 0 && d <= 7 }).length}` : null,
        ``,
        `${'─'.repeat(50)}`,
