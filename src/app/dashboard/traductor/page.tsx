...
export default function Traductor() {
  ...
  return (
    <div style={{ padding:'32px', fontFamily:'sans-serif' }}>
      ...
      <p style={{ color:'#88
cat > src/app/dashboard/traductor/page.tsx << 'FINARCHIVO'
'use client'
import { useState } from 'react'
...
export default function Traductor() {
  ...
  return (
    <div style={{ padding:'32px', fontFamily:'sans-serif' }}>
      <h1 style={{ color:'#0F2447', fontSize:'24px', fontWeight:700, margin:'0 0 4px' }}>Traductor Legal</h1>
      <p style={{ color:'#888', margin:'0 0 24px' }}>Traduccion especializada en terminos juridicos — Gemini 1.5 Flash</p>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'1fr 300px', gap:'24px' }}>
        <div style={{ background:'white', borderRadius:'16px', padding:'24px', boxShadow:'0 1px 4px rgba(0,0,0,0.06)', border:'1px solid #F0F0F0' }}>
          <div style={{ display:'flex', alignItems:'center', gap:'12px', marginBottom:'20px' }}>
            ...
            <button onClick={() => { const t=origen; setOrigen(destino); setDestino(t); setTraduccion('') }}
              style={{ padding:'10px 16px', borderRadius:'8px', border:'1.5px solid #E8E8E8', background:'white', cursor:'pointer', fontSize:'18px' }}>⇄</button>
          ...
          <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:'16px', marginBottom:'16px' }}>
            ...
            <div>
              ...
              <div style={{ width:'100%', padding:'14px', borderRadius:'10px', border:'1.5px solid #E8E8E8', fontSize:'13px', color:'#555', lineHeight:'1.7', minHeight:'320px', background:'#FAFAFA', boxSizing:'border-box' }}>
                {traduciendo ? <span style={{ color:'#888' }}>Traduciendo...</span> : traduccion || <span style={{ color:'#ccc', fontStyle:'italic' }}>La traduccion aparecera aqui...</span>}
              </div>
            </div>
          </div>

          <div style={{ display:'flex', gap:'10px' }}>
            <button onClick={traducir} disabled={traduciendo||!texto.trim()}
              style={{ background:texto.trim()?'#E8321A':'#E8E8E8', color:texto.trim()?'white':'#aaa', border:'none', padding:'12px 28px', borderRadius:'8px', fontSize:'14px', fontWeight:700, cursor:texto.trim()?'pointer':'default' }}>
              {traduciendo ? 'Traduciendo...' : 'Traducir'}
            </button>
            {traduccion && (
              <button onClick={() => navigator.clipboard?.writeText(traduccion)}
        ...
        <div style={{ display:'flex', flexDirection:'column', gap:'16px' }}>
          <div style={{ background:'white', borderRadius:'16px', padding:'20px', boxShadow:'0 1px 4px rgba(0,0,0,0.06)', border:'1px solid #F0F0F0' }}>
            ...
            {historial.length === 0 ? (
              <p style={{ color:'#888', fontSize:'13px', margin:0 }}>Sin traducciones aun</p>
            ) : historial.map((h,i) => (
          ...
          <div style={{ background:'#F8F8F8', borderRadius:'16px', padding:'20px', border:'1px solid #F0F0F0' }}>
            <h3 style={{ color:'#0F2447', fontSize:'13px', fontWeight:700, margin:'0 0 10px' }}>Idiomas disponibles</h3>
            <div style={{ display:'flex', flexWrap:'wrap', gap:'6px' }}>
              {idiomas.map((id,i) => (
                <span key={i} style={{ background:'white', color:'#0F2447', fontSize:'11px', fontWeight:600, padding:'3px 8px', borderRadius:'10px', border:'1px solid #E8E8E8' }}>{id}</span>
              ))}
            </div>
          </div>
        </div>
